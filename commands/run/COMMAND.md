...
echo "Starting robot dataflow..."
dora run robot/dataflow.yml
...
def run_dataflow(yaml_path):
    """Run dataflow and handle signals"""
...
if __name__ == "__main__":
    exit(run_dataflow('dataflow.yml'))
```
...
test: build
	dora run test-dataflow.yml
```

### Docker exec
```bash
# Run inside container
docker exec -it my-container dora run /app/dataflow.yml
```
